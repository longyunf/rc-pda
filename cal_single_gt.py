...
from fuse_lidar7c import merge_lidar, get_tracks_forward, update_key_tracks, update_non_key_tracks, cal_depthMap_flow, filter_occlusion, lidarFlow2uv



def get_tracks_scene(nusc, sample_idx):
    '''
    obtain all bounding boxes of all Lidar frames in a scenes where a sample (referred by sample_idx) exists
    '''
...
if __name__ == '__main__':
    ...
    for sample_idx in sample_indices[start_idx: end_idx+1]:
        ...
        x1, y1, depth1, x2, y2, depth2, msk_low_h = merge_lidar(nusc, sample_idx, n_backward, n_forward, box_tracks, n_skip)
        
        depth_map, flow_lidar, msk_map_low_h = cal_depthMap_flow(x1, y1, depth1, x2, y2, depth2, msk_low_h, downsample_scale=4, y_cutoff=33)

        depth_map, msk_map_low_h, flow_lidar = filter_occlusion(depth_map, msk_map_low_h, flow_lidar, flow_im, thres=3)
        ...
        np.save(join(dir_data_out, '%05d_gt_one.npy' % sample_idx), gt)        
        ...
        # plt.figure()    
        # plt.imshow(np.squeeze(depth_map), cmap='jet')
        # plt.colorbar()
        # plt.title('Lidar')
        # plt.show()        
        # pass
