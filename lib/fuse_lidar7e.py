'''
registrate Lidar points based on ego-vehicle pose and bounding boxes of object

two camera images im1 and im2 are 2nd ad 3rd images after the image corresponding to key Lidar frame

Lidar flow: im1 -> im2
depth: depth for im1

compute a image mask for Lidar points above ground and below 3 meters

compute masks for points in box (vehicle or other objects) based on 3D bounding boxes
    
use segmentaion and bounding box to remove background points
...
def merge_lidar(nusc, sample_idx, n_backward, n_forward, box_tracks, n_skip=0):
    ...
    def current_to_global_at_ref_time(nusc, current_sd_token, ref_sd_token, box_tracks):
        ...
        for instance_token in box_tracks[ref_sd_token]:
            if instance_token in box_tracks[current_sd_token].keys():
                ...
                if 'vehicle' in box['category_name']:   # all points in bbox; we can choose only vehicle points or moving points
    ...
    corner_pc = cal_box_corners_in_global(nusc, cam_token1, box_tracks)   # bounding box corners; can choose object class or moving status
...
if __name__ == '__main__':
    ...
    depth_map1, flow, msk_map_low_h, msk_map_in = cal_depthMap_flow(x1, y1, depth1, x2, y2, depth2, msk_low_h, msk_in, downsample_scale=4, y_cutoff=33)
     
    # msk_bbox = cal_msk_bbox(x_cn, y_cn, msk_cn, downsample_scale=4, y_cutoff=33)  # mask for projected rectangle of 3D bounding box
    
    seg = np.load(join(out_dir, '%05d_seg.npy' % sample_idx))
    ...
    # plt.figure()
    # plt.imshow(im1 * seg[...,None])
    # plt.title('segmentation')
    # plt.show()
    
    
    plt_depth_on_im(depth_map1, im1, title='before filtering')   
    # plt_flow_on_im(flow, im1, skip=2, title = 'lidar flow')
       
    # plt.figure()
    # plt.imshow(depth_map1, cmap='jet') 
    # plt.title('depth before filtering')
    
    
    
    # depth_map1, msk_d1 = filter_occlusion_by_bbox(depth_map1, seg, msk_map_in, x_cn, y_cn, depth_cn, msk_cn, downsample_scale=4, y_cutoff=33)
    
    # flow, msk_map_low_h = flow * msk_d1[..., None], msk_map_low_h * msk_d1

    # plt_depth_on_im(depth_map1, im1, title='filtered by bounding box')
    
    
    im_flow = np.load(join(out_dir, '%05d_flow.npy' % sample_idx))
    plt_flow_on_im(im_flow, im1, skip=5, title='optical flow')
    ...
    plt_depth_on_im(error > thres_occ, im1, title='msk for large flow difference')
    
    plt_depth_on_im(error, im1, title='error')

    
    depth_map1, msk_map_low_h, msk_map_in, flow = filter_occlusion(depth_map1, msk_map_low_h, msk_map_in, flow, im_flow, thres = thres_occ)
    
    plt_depth_on_im(depth_map1, im1, title='after flow filtering')
    
    # plt.figure()
    # plt.imshow(depth_map1, cmap='jet') 
    # plt.title('depth after filtering')
        
    # plt.figure()
    # plt.imshow(msk_map_low_h)
    # plt.title('low height')
    
    # plt.figure()
    # plt.imshow(msk_bbox)
    # plt.title('bounding box')
    
    plt_depth_on_im(depth_map1 * msk_map_low_h, im1, title='low height')    
    plt_depth_on_im(depth_map1 * msk_map_in, im1, title='points in bounding box')
    
    # plt.figure()
    # plt.imshow(im1 * msk_bbox[...,None])
    # plt.title('bounding box')
